...
class Explosion{
	 ...
	 */
	public function __construct(Position $center, float $size, $what = null, bool $dropItem = true, $arsonist = null){
		if(!$center->isValid()){
	...
	public function explodeB() : bool{
		...
		if($this->what instanceof Entity){
			$this->level->getServer()->getPluginManager()->callEvent($ev = new EntityExplodeEvent($this->what, $this->source, $this->affectedBlocks, $yield, $this->arsonist));
			if($ev->isCancelled()){
				return false;
			}else{
				$yield = $ev->getYield();
