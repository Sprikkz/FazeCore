...
class Level implements ChunkManager, Metadatable{
    ...
    public function useBreakOn(Vector3 $vector, Item &$item = null, Player $player = null, bool $createParticles = false) : bool{
        ...
        if ($tile !== null) {
            if ($tile instanceof Container) {
                ...
                // The queue is not processed immediately, that's where we get the item delay in FloatingInventory
                // Deleting only the last transaction won't work, because the player can make several transactions
    ...
    public function addTile(Tile $tile) {
		...
		if(isset($this->chunks[$hash = Level::chunkHash($chunkX, $chunkZ)])){
            ...
            if ($this->loadChunk($chunkX, $chunkZ, false) === true) {
                if(isset($this->chunks[$hash = Level::chunkHash($chunkX, $chunkZ)])){
                    $this->chunks[$hash]->addTile($tile);
                }else{
                    $this->server->getLogger()->warning("Attempted to create tile " . get_class($tile) . " in unloaded chunk $chunkX $chunkZ, removing this shit..");
                    $vec = new Vector3($tile->x, $tile->y, $tile->z);
                    $this->setBlock($vec, new Air());
                    return;
                }
            }else{
                $this->server->getLogger()->warning("Attempted to create tile " . get_class($tile) . " in unloaded chunk $chunkX $chunkZ, removing this shit..");
