...
use pocketmine\achievement\Achievement;
...
class Server{
	...
	*/
	...
	public function getPocketMineVersion(){
		return EssentialInfo::MINECRAFT_VERSION;
	}

	public function getFormattedVersion($prefix = ""){
		return (EssentialInfo::MINECRAFT_VERSION !== ""? $prefix . EssentialInfo::MINECRAFT_VERSION : "");
	}
	...
	public function getApiVersion(){
		return EssentialInfo::API_VERSION;
	}
	
	public function getExtraApiVersion(){
		return EssentialInfo::EXTRA_API_VERSION;
	}
	 ...
	 * @return bool
	 */
	public function isMobs(){
		return $this->getConfigBoolean("auto-spawn-mobs", false);
	}
	...
	public function unloadLevel(Level $level, $forceUnload = false){
		if($level === $this->getDefaultLevel() and !$forceUnload){
			throw new \InvalidStateException("The world cannot be unloaded during operation by default, please switch worlds.");
		}
	...
	public function loadLevel($name){
        ...
        $level = new Level($this, $name, $path, $providerClass);
	...
	public function generateLevel($name, $seed = null, $generator = null, $options = []){
		...
		if(($provider = LevelProviderManager::getProviderByName($providerName = $this->getProperty("level-settings.default-format", "pmanvil"))) === null){
			...
			if($provider === null){
				throw new \InvalidStateException("The default level provider is not registered.");
			}
		}
		...
		//$this->getLogger()->info($this->getLanguage()->translateString("pocketmine.level.backgroundGeneration", [$name]));
		$this->getLogger()->info("Generating spawn area of the world '{$name}'...");
	...
	public static function getInstance() : Server{
		if(self::$instance === null){
			throw new \RuntimeException("Attempt to get a server instance outside of the server thread.");
		}
	...
	public function loadAdvancedConfig(){
		...
		$this->transferPlayers = $this->getAdvancedProperty("server.transfer-players", false);
		$this->transferIP = $this->getAdvancedProperty("server.transfer-ip", "server.net");
		$this->transferPort = $this->getAdvancedProperty("server.transfer-port", 19132);
		$this->dserverConfig = [
...
§c░░░░░        ░░░░░░░░  ░░░░░░░░░  ░░░░░░    §f░░░░░░░░░   ░░░░░░  ░░░░░      ░░░░░░§r                                                               
 ...
 > §cMinecraft PE Version: §f1.1.x
 > §cPHP Version: §f8.0.28
 > §cOperating System: §fWINNT
 > §cSystem Architecture: §f64-bit

 > §cBuild Date: §f2024-11-30

 > §cDeveloper: §fSprikkz
 > §cThis is a fork of §fPocketMine-MP.
');
            $this->logger->info('Loading configuration, reading data...');
			...
			if(file_exists(\pocketmine\PATH . "src/pocketmine/resources/essential_$lang.yml")){
				$content = file_get_contents($file = \pocketmine\PATH . "src/pocketmine/resources/essential_$lang.yml");
			}else{
				$content = file_get_contents($file = \pocketmine\PATH . "src/pocketmine/resources/essential_rus.yml");
			}

			if(!file_exists($this->dataPath . "essential.yml")){
				@file_put_contents($this->dataPath . "essential.yml", $content);
			}
			$internelConfig = new Config($file, Config::YAML, []);
			$this->advancedConfig = new Config($this->dataPath . "essential.yml", Config::YAML, []);
			...
			if(!extension_loaded("openssl")){
				...
				$this->logger->info('Xbox Live authentication has been disabled.');
			}
			...
			if(!$this->getProperty("settings.enable-dev-builds", false)){
                $this->logger->warning("Unable to start the server due to configuration settings.");
                $this->logger->warning("You are running a DEVELOPMENT build of Essential, but your configuration does not allow running such builds.");
                $this->logger->warning("Development builds may contain unexpected bugs, crashes, break your plugins, corrupt all your data, and much more.");
                $this->logger->warning("If you are not a developer and do not know what you are doing, PLEASE AVOID using test builds.");
                $this->logger->warning("To use this build, set 'enable-dev-builds' to 'true' in pocketmine.yml.");
			    @Utils::kill(getmypid());
			}
			...
			if($this->networkCompressionLevel < 1 or $this->networkCompressionLevel > 9){
                $this->logger->warning("Invalid network compression level $this->networkCompressionLevel set, defaulting to 6.");
			...
			if($this->getConfigBoolean("enable-rcon", false)){
				try{
                    ...
                    $this->getLogger()->critical("RCON Interface failed to start: " . $e->getMessage());
                }
			}
			...
			if(((int) ini_get('zend.assertions')) !== -1){
                $this->logger->warning("Debug assertions are enabled, this may affect performance. To disable them, set `zend.assertions = -1` in php.ini.");
            }
            ...
            $this->logger->warning("This software is distributed under the LGPL license.");
	...
	public function batchPackets(array $players, array $packets, bool $forceSync = false, bool $immediate = false){
		if(count($packets) === 0){
			throw new \InvalidArgumentException("Cannot send an empty batch.");
		}
	...
	public function reload(){
		...
		$this->logger->info("Reloading properties and configuration...");
	...
	public function forceShutdown(){
		...
		try{
			...
			if($this->getProperty("network.upnp-forwarding", false)){
				$this->logger->info("[UPnP] Removing forwarded port...");
			...
			if($this->transferPlayers){
			    foreach($this->players as $player){
				    $player->transfer($this->transferIP, $this->transferPort);
			    }
			} else
			    foreach($this->players as $player){
			        $player->close($player->getLeaveMessage(), "§cThe server is restarting or shut down for maintenance!");
			    }
			...
			if($this->properties !== null and $this->properties->hasChanged()){
				$this->getLogger()->debug("Saving settings...");
				$this->properties->save();
			}
			...
			if($this->network instanceof Network){
				...
				foreach($this->network->getInterfaces() as $interface){
					$this->getLogger()->debug("Shutting down network interface " . get_class($interface));
					$interface->shutdown();
			...
			$this->logger->logException($e);
			$this->logger->emergency("The server crashed while shutting down. Killing the process...");
			@Utils::kill(getmypid());
		}
	...
	public function start(){
        ...
        $this->logger->info($this->getLanguage()->translateString('pocketmine.server.startFinishedRun', [round(microtime(true) - \pocketmine\START_TIME, 3)]));

        if(!file_exists($this->getPluginPath() . DIRECTORY_SEPARATOR . "Essential"))
            @mkdir($this->getPluginPath() . DIRECTORY_SEPARATOR . "Essential");
	...
	public function crashDump(){
		...
		try{
			...
			if($this->getProperty("auto-report.enabled", false) !== false){
				...
				if(file_exists($stamp) and !($report = (filemtime($stamp) + $crashInterval < time()))){
                    $this->logger->debug("Crash dump not sent due to the last crash being less than $crashInterval seconds ago.");
                }
				...
				if(is_string($plugin)){
					...
					if($p instanceof Plugin and !($p->getPluginLoader() instanceof PharPluginLoader)){
                        $this->logger->debug("Emergency dump is not being sent due to a plugin other than phar");
						$report = false;
					}
		...
		if($spacing > 0){
            echo "--- Waiting $spacing seconds before the next restart. (You can safely terminate the process) ---" . PHP_EOL;
            sleep($spacing);
		}
		@Utils::kill(getmypid());
		exit(1);
	...
	private function checkTickUpdates(int $currentTick, float $tickTime) : void{
		...
		foreach($this->levels as $k => $level){
			...
			if($tickMs >= 50){
                $this->getLogger()->debug(sprintf("World \"%s\" took too long for a tick: %gms (%g ticks)", $level->getName(), $tickMs, round($tickMs / 50, 2)));
            }
		}
	}
	...
	private function tick() : void{
		...
		if(($this->tickCounter % 20) === 0){
            ...
            if($this->dserverConfig["enable"] and $this->dserverConfig["motdPlayers"]){
                ...
                //TODO: (Check if it's full, different status colors) 检测是否爆满,不同状态颜色
            }
		}

		if($this->autoSave and ++$this->autoSaveTicker >= $this->autoSaveTicks){
			$this->autoSaveTicker = 0;
			$this->getLogger()->debug("(Autosave) Saving world...");
			$start = microtime(true);
			$this->doAutoSave();
			$time = (microtime(true) - $start);
			$this->getLogger()->debug("(Autosave) Save completed in " . ($time >= 1 ? round($time, 3) . "с" : round($time * 1000) . "мс"));
		}
