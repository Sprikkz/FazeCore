...
class Human extends Creature implements ProjectileSource, InventoryHolder {
	...
	public  function addToCooldown(){
		$this->useCooldown = time() + 1;
		if($this instanceof Player) $this->sendMessage("§e> Тотем можно будет использовать снова через 1 секунд.");
	}

	public function isInCooldown() : bool{
		return $this->useCooldown > time();
	}
		...
		if($source->getCause() !== EntityDamageEvent::CAUSE_VOID){
			...
			if($damage > 0 && ($this->getHealth() - $damage) <= 0){
				...
				if($isOffHand
					...
					if($this instanceof Player and $this->isInCooldown()){
						$timeLeft = function() : string{
							$time = $this->useCooldown - time();
						...
						$this->sendMessage("§7> §cТвой тотем ещё перезаряжается §6". $timeLeft());
					...
					if(!$event->isCancelled()){
						...
						$this->addEffect(Effect::getEffect(Effect::REGENERATION)->setDuration(3 * 45)->setAmplifier(0));
						$this->addEffect(Effect::getEffect(Effect::ABSORPTION)->setDuration(3 * 5)->setAmplifier(0));
						$this->addEffect(Effect::getEffect(Effect::FIRE_RESISTANCE)->setDuration(3 * 40));
