...
class PluginDescription {
    ...
    private function loadMap(array $plugin){
		...
		if(preg_match('/^[A-Za-z0-9 _.-]+$/', $this->name) === 0){
			throw new PluginException("Invalid plugin description name");
		}
		$this->name = str_replace(" ", "_", $this->name);
		$this->version = $plugin["version"];
		$this->main = $plugin["main"];
		$this->api = !is_array($plugin["api"]) ? [$plugin["api"]] : $plugin["api"];
		if(!isset($plugin["extraapi"])){
			$this->extraapi = ["1.0.0"];
		}else{
			$this->extraapi = !is_array($plugin["extraapi"]) ? [$plugin["extraapi"]] : $plugin["extraapi"];
		}

		if(stripos($this->main, "pocketmine\\") === 0){
			throw new PluginException("Invalid main plugin description, cannot run in PocketMine namespace.");
		}
		...
		if(isset($plugin["load"])){
			...
			if(!defined(PluginLoadOrder::class . "::" . $order)){
				throw new PluginException("Invalid plugin description loading");
			}else{
	 ...
	 * @return array
	 */
	public function getCompatibleExtraApis(){
