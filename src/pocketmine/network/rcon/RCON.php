...
/**
 * Implementation of the original RCON protocol to allow remote console commands.
...
class RCON{
	...
	public function __construct(Server $server, string $password, int $port = 19132, string $interface = "0.0.0.0", int $maxClients = 50){
		$this->server = $server;
		$this->server->getLogger()->info("Starting the Remote Control Listener");
		if($password === ""){
		...
		if(!socket_set_option($this->socket, SOL_SOCKET, SO_REUSEADDR, 1)){
			throw new \RuntimeException("Failed to set parameter on socket: " . trim(socket_strerror(socket_last_error())));
		}
		...
		$this->server->getLogger()->info("RCON is working $addr:$port");
	}
