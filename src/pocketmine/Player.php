...
use pocketmine\player\DataPacketHandler;
...
class Player extends Human implements CommandSender, InventoryHolder, ChunkLoader, IPlayer {
	...
	public function checkSkin(){
        ...
        $this->close("", "Invisible skins are not allowed on our server!");
		return false;
	}
	...
	public function __construct(SourceInterface $interface, $ip, $port){
		...
		$this->dataPacketHandler = new DataPacketHandler($this);
	}
	...
	public function setInteractiveTag(TextContainer $tag)
	{
		$this->setDataProperty(
			self::DATA_INTERACTIVE_TAG,
			self::DATA_TYPE_STRING,
			$this->server->getLanguage()->translate($tag),
		);
	}

	public function removeInteractiveTag() {
		$this->setDataProperty(
			self::DATA_INTERACTIVE_TAG,
			self::DATA_TYPE_STRING,
			null,
    ...
    public function sendPing() {
        if ($this->ping <= 100) {
            $this->sendMessage("§fGreat connection §a({$this->ping}ms)");
        } elseif ($this->ping <= 150) {
            $this->sendMessage("§fGood connection §6({$this->ping}ms)");
        } else {
            $this->sendMessage("§fTerrible connection §c({$this->ping}ms)");
        }
    }
	...
	protected function doFirstSpawn(){
		...
		if($this->getPing() >= 5000){
            $this->close("", "Ping is too high! Please rejoin later.");
        }
    ...
    public function dataPacket(DataPacket $packet, bool $needACK = false, bool $immediate = false){
        ...
        $this->dupe[$this->getName()] = $packet->getName();
        if($packet->getName() == 'EntityEventPacket') {
            if($packet instanceof EntityEventPacket) {
                if ($packet->data >= 999) {
                    $this->server->getLogger()->info('Player ' . $this->getName() . ', sent packet: EntityEventPacket. His DATA: ' . $packet->data . ', EID: ' . $packet->eid . ', EVENT: ' . $packet->event . '.');
                    $this->close('', '§cYou have been kicked by the protection!');
                }
            }
        }
        if (($packet instanceof ChangeDimensionPacket && $this->dupe[$this->getName()] instanceof AddItemPacket) or ($packet instanceof AddItemPacket && $this->dupe[$this->getName()] instanceof ChangeDimensionPacket)) {
            $this->server->getLogger()->info('Player ' . $this->getName() . ', sent packet ' . $packet->getName() . '. The system suspected a dupe.');
            $this->close($this->getLeaveMessage(), self::DISCONNECT_SCREEN_KICK);
        }

        // Basic safety restriction. TODO: improve this
        if(!$this->loggedIn and !$packet->canBeSentBeforeLogin()){
            throw new \InvalidArgumentException("Attempted to send packet: " . get_class($packet) . ", player:  " . $this->getName() . " too early.");
        }
    ...
    protected function processLogin(){
        ...
        if(!$this->server->isWhitelisted($this->getLowerCaseName())){
            $this->close('', 'The server is under maintenance!');

            return;
        }
        // Check if the player is banned by different methods
        elseif($this->isBanned() or $this->server->getIPBans()->isBanned($this->getAddress()) or $this->server->getCIDBans()->isBanned($this->randomClientId)){
            $this->close($this->getLeaveMessage(), TextFormat::RED . "§cYou are banned");

            return;
        }

        // Check if a player with the same username or unique ID is already online
        foreach($this->server->getOnlinePlayers() as $p){
            if($p !== $this and ($p->iusername === $this->iusername or $this->getUniqueId()->equals($p->getUniqueId()))){
                $this->close($this->getLeaveMessage(), "A player with this username is already online, please change your username!");

                return;
            }
        }

        // If the player is already logged in, block the address temporarily
        if ($this->loggedIn) {
        ...
        // Retrieve player data and initialize necessary attributes
        ...
        // Set the player's gamemode based on the server's settings
        ...
        // Allow flight if the player is in creative mode
        $this->allowFlight = $this->isCreative();
        // Allow movement if the player is in spectator mode or cheats are allowed
        $this->keepMovement = $this->isSpectator() || $this->allowMovementCheats();

        // Set the player's level and position
        ...
        // Check if the named tag is correctly passed, otherwise kick the player
        if(!($this->namedtag instanceof CompoundTag)){
            $this->close($this->getLeaveMessage(), "Player data is invalid!");
        ...
        // Send resource pack information
    ...
    protected function completeLoginSequence(){
		...
		$pdevice = $this->getDeviceModel();
		$pnick = $this->getName();
		$pprotocol = $this->getProtocol();
		$pclient = $this->getClientId();
		$pip = $this->getAddress();
		$pport = $this->getPort();
		$plevel = $this->level->getName();
		
		if($pdevice == "System manufacturer System Product Name") {
		    $pdevice = "Windows 10";
		}else{
		    $pdevice = $this->getDeviceModel();
		}

        if(!is_dir("./players/$pnick")){
		    @mkdir("./players/$pnick");
		}

	    if(!file_exists("./players/$pnick/$pnick.log")){
			$f = fopen("./players/$pnick/$pnick.log","a+");
			fwrite($f,"Nick: $pnick\nDevice: $pdevice\nProtocol: $pprotocol\nCID: $pclient\nAddress: $pip:$pport\nLevel: $plevel\n");
			fclose($f);
		}

        $this->server->getLogger()->info("§fNickname: §b$pnick §7| §fDevice: §b$pdevice §7| §fProtocol: §b$pprotocol §7| §fCID: §b$pclient §7| §fAddress: §b$pip:$pport §7| §fWorld: §b$plevel");
	...
	/**
	 * TODO: counting all packets, transferring the packet to the array, possible queue of them.
	 */
	public function isFloodBatch($ip, $port){
		if(!empty($ip) and !empty($port)){
		    ...
		    if($this->handleQueue[$ip][$port]['packets'] > 499){
		    	$this->isFlooding = true;
		    	
			    $this->server->getLogger()->warning("Too many BatchPackets detected from IP: {$ip}");
			    $this->server->getLogger()->warning("BatchPS: {$this->handleQueue[$ip][$port]['packets']}");

			    $this->close("", "Suspicious traffic detected", true);
	...
	public function handleDataPacket(DataPacket $packet){
		...
		if ($this->dataPacketHandler->handle($packet)) return;
		...
		switch($packet::NETWORK_ID){
            ...
            case ProtocolInfo::LOGIN_PACKET:
				...
				/*
                $split = explode(':', $packet->serverAddress);
                $hip = gethostbyname($split[0]);
                if((int) $split[1] != Server::getInstance()->getPort()){
                    $this->sendPlayStatus(PlayStatusPacket::LOGIN_SUCCESS);

                    $this->getServer()->getNetwork()->blockAddress($this->getAddress(), 500);
                    $this->server->getLogger()->warning("Заблокирована попытка входа с прокси: {$this->getAddress()}:{$this->getPort()} (port).");
                }else if($hip != $this->getServer()->serverIp && Internet::isNonLocalAddress($this->ip)){
                    $this->sendPlayStatus(PlayStatusPacket::LOGIN_SUCCESS);

                    $this->getServer()->getNetwork()->blockAddress($this->getAddress(), 500);
                    $this->server->getLogger()->warning("Заблокирована попытка входа с прокси: {$this->getAddress()}:{$this->getPort()} (ip).");
                }
                */
		        ...
		        if ($this->getPort() === $this->server->getPort()) {
		        	$this->close("", "disconnectionScreen.invalidClientData");
					...
					case PlayerActionPacket::ACTION_RELEASE_ITEM:
						if($this->startAction > -1 and $this->getDataFlag(self::DATA_FLAGS, self::DATA_FLAG_ACTION)){
							if($this->inventory->getItemInHand()->getId() === Item::BOW){
								...
								if($f < 0.1 or $diff < 5 or $this->isSpectator()){
									$ev->setCancelled();
								}
			...
			case ProtocolInfo::INTERACT_PACKET:
				...
				if ((round(microtime(true) * 1000) - $this->antiClicker) < 100) {
					$this->antiClicker = round(microtime(true) * 1000);
					break;
				}
			...
			case ProtocolInfo::COMMAND_STEP_PACKET:
                ...
                $this->server->getLogger()->info("§fPlayer §b{$this->getName()} §fentered the command §7/{$commandText}");
                if($ev->isCancelled()){
					break;
				}
			...
			case ProtocolInfo::TEXT_PACKET:
				...
				if($packet->type === TextPacket::TYPE_CHAT){
					/*if ($packet->source !== $this->username) {
					    $this->server->getNetwork()->blockAddress($this->getAddress(), 1200);//TODO: needs testing
					    break;
				    }*/ 
					if(strlen($packet->message) > 350){
						$this->server->getNetwork()->blockAddress($this->getAddress(), 1202); //TODO: give a chance to players who have typed 350 characters..
                        break;
					}
			...
			case ProtocolInfo::REQUEST_CHUNK_RADIUS_PACKET:
			    ...
			    if ($radius <= 48 and $radius >= 4) {
			    	if ($this->getDeviceOS() == 1 or $this->getDeviceOS() == 2) {
			    		if ($radius > 22) {
			    			$this->server->getNetwork()->blockAddress($this->getAddress(), 1500); //Временная защита от прокси
			    			$this->server->getLogger()->notice("The player $this->username using danger soft.");
			    			break;
			    		}
			    	}
	...
	public final function close($message = "", string $reason = "generic reason", bool $notify = true){
		if($this->isConnected() and !$this->closed){
			...
			$this->server->getLogger()->info($this->getServer()->getLanguage()->translateString("pocketmine.player.logOut", [
				TextFormat::YELLOW . $this->getName() . TextFormat::WHITE,
				$this->ip,
				$this->port,
