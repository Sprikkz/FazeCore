...
class BanList {
	...
	public function load(){
		...
		if(is_resource($fp)){
			while(($line = fgets($fp)) !== false){
				if($line[0] !== "#"){
					try{
						...
						$logger->critical("Unable to parse the disallow entry from the string \"$line\": " . $e->getMessage());
			...
			MainLogger::getLogger()->error("Failed to load ban list");
		}
	}
	...
	public function save($flag = true){
		...
		if(is_resource($fp)){
			...
			MainLogger::getLogger()->error("Failed to save ban list");
		}
	}
