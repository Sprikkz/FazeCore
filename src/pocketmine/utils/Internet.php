...
class Internet{
	...
	/**
	 * Gets external IP address using external service, it is cached
	 *
	...
	/**
     * Returns the IP address of the machine's internal network. If the machine is not behind a router, this may be the same.
     * as external IP.
	 *
     ...
     * NOTE: This is a blocking operation and may take a significant amount of time. It is not recommended to use this method on the main thread.
	 *
	...
	/**
     * General cURL shortcut function.
     * NOTE: This is a blocking operation and may take a significant amount of time. It is not recommended to use this method on the main thread.
	 *
	...
	public static function simpleCurl(string $page, $timeout = 10, array $extraHeaders = [], array $extraOpts = [], callable $onSuccess = null){
		if(!self::$online){
			throw new InternetException("Unable to complete web request in offline mode");
		}

		$ch = curl_init($page);
		if($ch === false){
			throw new InternetException("Failed to create new cURL session");
		}
		...
		curl_setopt_array($ch, $extraOpts + [
			...
			CURLOPT_HTTPHEADER => array_merge(["User-Agent: Mozilla/5.0 (Windows NT 6.1; WOW64; rv:12.0) Gecko/20100101 Firefox/12.0 " . EssentialInfo::NAME . "/" . EssentialInfo::MINECRAFT_VERSION], $extraHeaders),
			CURLOPT_HEADER => true
		]);
		try{
			$raw = curl_exec($ch);
