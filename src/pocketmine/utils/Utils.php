...
class Utils{
	...
	public static function getRealMemoryUsage(){
		...
		if(Utils::getOS() === "linux" or Utils::getOS() === "android"){
			...
			foreach($mappings as $line){
				if(preg_match("#([a-z0-9]+)\\-([a-z0-9]+) [rwxp\\-]{4} [a-z0-9]+ [^\\[]*\\[([A-z0-9]+)\\]#", trim($line), $matches) > 0){
	...
	public static function getTrace($start = 0, $trace = null){
		...
		for($i = (int) $start; isset($trace[$i]); ++$i, ++$j){
			...
			$messages[] = "#$j " . (isset($trace[$i]["file"]) ? self::cleanPath($trace[$i]["file"]) : "") . "(" . ($trace[$i]["line"] ?? "") . "): " . (isset($trace[$i]["class"]) ? $trace[$i]["class"] . (($trace[$i]["type"] === "dynamic" or $trace[$i]["type"] === "->") ? "->" : "::") : "") . $trace[$i]["function"] . "(" . Utils::printable($params) . ")";
		}
	...
	public static function printableTrace(array $trace, int $maxStringLength = 80) : array{
		...
		for($i = 0; isset($trace[$i]); ++$i){
			...
			$messages[] = "#$i " . (isset($trace[$i]["file"]) ? self::cleanPath($trace[$i]["file"]) : "") . "(" . ($trace[$i]["line"] ?? "") . "): " . (isset($trace[$i]["class"]) ? $trace[$i]["class"] . (($trace[$i]["type"] === "dynamic" or $trace[$i]["type"] === "->") ? "->" : "::") : "") . $trace[$i]["function"] . "(" . Utils::printable($params) . ")";
		}
		return $messages;
	}
