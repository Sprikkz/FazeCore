...
class GarbageCollectorCommand extends VanillaCommand {
	...
	public function execute(CommandSender $sender, $currentAlias, array $args){
        ...
        $sender->sendMessage("§c--- §fCleanup report §c---");
        $sender->sendMessage("§fChunks:§e " . \number_format($chunksCollected));
        $sender->sendMessage("§fEntities:§e " . \number_format($entitiesCollected));
        $sender->sendMessage("§fTiles:§e " . \number_format($tilesCollected));
        $sender->sendMessage("§fCycles:§e " . \number_format($cyclesCollected));
        $sender->sendMessage("§fMemory:§e " . \number_format(\round((($memory - \memory_get_usage()) / 1024) / 1024, 2), 2) . " MB");
        return true;
	}
}
