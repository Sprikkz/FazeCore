...
class Session{
	...
	public function update(float $time) : void{
		...
		if($this->state === self::STATE_DISCONNECT_PENDING || $this->state === self::STATE_DISCONNECT_NOTIFIED){
			...
			if((count($this->sendQueue->packets) === 0 and count($this->ACKQueue) === 0 and count($this->NACKQueue) === 0 and count($this->packetToSend) === 0 and count($this->recoveryQueue) === 0)){
				if($this->state === self::STATE_DISCONNECT_PENDING){
					...
					$this->sessionManager->getLogger()->debug("All pending traffic flushed, sent disconnect notification");
				}else{
					$this->state = self::STATE_DISCONNECTED;
					$this->sessionManager->getLogger()->debug("Client cleanly disconnected, marking session for destruction");
					return;
				}
			}elseif($this->disconnectionTime + 10 < $time){
				$this->state = self::STATE_DISCONNECTED;
				$this->sessionManager->getLogger()->debug("Timeout during graceful disconnect, forcibly closing session");
				return;
			}
		}
	...
	private function handlePong(int $sendPingTime, int $sendPongTime) : void{
		$currentTime = $this->sessionManager->getRakNetTimeMS();
		if($currentTime < $sendPingTime){
			$this->sessionManager->getLogger()->debug("Received invalid pong: timestamp is in the future by " . ($sendPingTime - $currentTime) . " ms");
		}else{
	...
	public function initiateDisconnect(string $reason) : void{
		if($this->isConnected()){
		    ...
		    $this->sessionManager->streamClose($this->address->toString(), $reason);
		    $this->sessionManager->getLogger()->debug("Requesting graceful disconnect because \"$reason\"");
		}
	}
	...
	public function forciblyDisconnect(string $reason) : void{
		$this->state = self::STATE_DISCONNECTED;
		$this->sessionManager->streamClose($this->address->toString(), $reason);
		$this->sessionManager->getLogger()->debug("Forcibly disconnecting session due to \"$reason\"");
	}
	...
	private function onClientDisconnect() : void{
		...
		if($this->isConnected()){
			...
			$this->sessionManager->streamClose($this->address->toString(), "client disconnect");
		}
		$this->state = self::STATE_DISCONNECTED;
		$this->sessionManager->getLogger()->debug("Terminating session due to client disconnect");
	}
