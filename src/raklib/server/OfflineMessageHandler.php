	...
	public function handle(OfflineMessage $packet, InternetAddress $address) : bool{
		if($packet instanceof UnconnectedPing){
			...
			if($packet->serverAddress->port === $this->sessionManager->getPort() or !$this->sessionManager->portChecking){
				...
				$existingSession = $this->sessionManager->getSession($address);
				if($existingSession !== null && $existingSession->isConnected()){
					...
					//overwritten
					$this->sessionManager->getLogger()->debug("Not creating session for $address due to session already opened");
					return true;
				}
